...
elseif CLIENT then
	CreateClientConVar("rb_ds1_enabletrails",0,true,false,"Enables Sword Trails, can be quite laggy.")
	CreateClientConVar("rb_ds1_debuginfo",0,true,false,"Show various debug info on the bots.")
...
function meta:DS_SetWeaponTrail(mat, _time, size, flag)
	if GetConVar("rb_ds1_enabletrails"):GetInt() == 1 then
		local data = {}
		data.Mat = mat
		data.Owner = self
		data.Time = _time
...
function meta:DS_StopWeaponTrail()
    local data = {}
    data.Owner = self
